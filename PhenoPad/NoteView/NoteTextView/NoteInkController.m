...
@interface NoteInkController() <PhenotypeToolbarDelegate, PhenotypeToolbarDataSource>
...
@implementation NoteInkController
...
//        self.placeholder2 = [NSString stringWithString:NSLocalizedString( @"on the screen", @"")];
        ...
        /////phenotype bar
        self.phenoBar = [[PhenotypeToolbar alloc] initWithFrame:CGRectMake(0, 0, [self currentScreenBoundsDependOnOrientation].size.width, 40.0)
                                                           delegate:self
                                                         dataSource:self];
...
- (BOOL) recognizeInk:(BOOL)bErase
{
	...
	if ([delegate respondsToSelector:@selector(InkCollectorResultReady:theResult:)])
	{
        ...
        //[delegate lineByPosition:ptStroke[0].pt];
	}
    return YES;
}
...
- (bool) whetherInsert
{
    if(strokeLen == 0)
        return false;
    if(ptStroke[strokeLen-1].pt.y - ptStroke[0].pt.y > 80)
    {
        if(ptStroke[strokeLen-1].pt.x - ptStroke[0].pt.x < 30 && ptStroke[strokeLen-1].pt.x - ptStroke[0].pt.x > -30)
        {
            return true;
        }
    }
...
-(void) processEndOfStroke:(BOOL)fromThread
{
    ...
    else // try shape
    {
        ...
        if(shape == SHAPE_LINE && [self whetherInsert]){
