...
- (id) initWithFrame:(CGRect)frame
{
    if((self = [super initWithFrame:frame]))
    {
        ...
        if ( b != YES )
        {
            [EditNoteView ensureDefaultSettings:YES];
        }
        else
        {
        ...
        case SHAPE_LINE:
            {
                if(isStylus){
                    ...
                    if (  edit != nil && nStrokes < 1 && strokeLen > 2 )
                    {
                        if([self whetherInsert]){
                            [self empty];
                            CGPoint start = ptStroke[0].pt;
                            CGPoint end = ptStroke[strokeLen-1].pt;
                            ...
                            [self.delegate openStylePopup:rt];
                            break;
                        }
